...
(function() {
  var Base, Init, S, Utils, W, dns, ejs, exec, fs, inquirer, ncp, nodecb, nodefn, path, readdirp, _,
  ...
  Init = (function(_super) {
    ...
    configure_options = function(opts) {
      ...
      if (!fs.existsSync(this.sprout_path)) {
        return W.reject(new Error("template '" + this.name + "' does not exist"));
      }
      if (!this.target) {
    ...
    merge_config_values_with_overrides = function() {
      this.config_values = _.assign(this.answers, this.overrides);
      return this.ejs_options = _.extend(this.config_values, {
        S: S
      });
    };
    ...
    replace_ejs = function() {
      ...
      return nodefn.call(readdirp, {
        root: this.target,
        directoryFilter: ignore_dirs
      }).tap((function(_this) {
        return function(res) {
          return res.files.map(function(f) {
            var out;
            out = ejs.render(fs.readFileSync(f.fullPath, 'utf8'), _this.ejs_options);
