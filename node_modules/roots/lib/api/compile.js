...
(function() {
  var Compile, Compiler, FSParser, W, _, fs, guard, keys, mkdirp, nodefn, sequence;
  ...
  Compile = (function() {
     ...
     */
    var after_ext_hook, after_hook, before_hook, create_folders, hook_method, process_files, purge_empty_folders, setup_extensions;
    ...
    Compile.prototype.exec = function() {
      ...
      return before_hook.call(this).then(setup_extensions.bind(this)).then(this.fs_parser.parse.bind(this.fs_parser))["with"](this).tap(create_folders).then(process_files).then(after_ext_hook).then(after_hook).then(purge_empty_folders).then(this.roots.emit.bind(this.roots, 'done'), function(err) {
        this.roots.emit('error', err);
        return W.reject(err);
      });
    };
    ...
    after_ext_hook = function() {
      return sequence(this.extensions.hooks('project_hooks.after'), this);
    };
    ...
    hook_method = function(hook) {
      ...
      if (Array.isArray(hook)) {
        hooks = hook.map((function(_this) {
          return function(h) {
            return nodefn.lift(_.partial(h, _this.roots).call(_this.roots));
          };
        })(this));
      } else if (typeof hook === 'function') {
        hooks = [nodefn.lift(_.partial(hook, this.roots).call(this.roots))];
      } else {
    ...
    create_folders = function(ast) {
      ...
      return nodefn.call(mkdirp, this.roots.config.output_path()).then(function() {
        return W.map(output_paths, guard(guard.n(1), (function(p) {
          return nodefn.call(mkdirp, p);
        })));
      });
    };
    ...
    process_files = function(ast) {
      var category, compile_task, ext, extfs, i, len, ordered, parallel, ref;
      ordered = [];
      parallel = [];
      compile_task = (function(_this) {
        return function(cat) {
          var ref;
          return W.map((ref = ast[cat]) != null ? ref : [], _this.compiler.compile.bind(_this.compiler, cat)).then(function() {
            return sequence(_this.extensions.hooks('category_hooks.after', cat), _this, cat);
          });
        };
      })(this);
      ref = this.extensions;
